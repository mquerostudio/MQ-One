...
int verify_values(char **values, char **expected, int n) {
  int i = 0;
...
static char *test_split_string_by_comma_ok() {
  int rv;
  char *test_str;
  char *values[24];
  ...
  mu_assert("should be able to split a comma seperated string",
            0 == verify_values(values, expected, rv));
  free(test_str);

  /* Empty values */
  test_str = strdup(",SOME,EMPTY,VALUES,,");
  rv = _split_string_by_comma(test_str, values, ARRAY_LENGTH(values));
  mu_assert("should return the correct number of values even when there is "
            "empty values (,,)",
            6 == rv);

  char *expected2[] = {"", "SOME", "EMPTY", "VALUES", "", ""};
  mu_assert("should be able to split empty values (,,)",
            0 == verify_values(values, expected2, rv));
  free(test_str);

  return 0;
}

static char *test_crop_sentence_ok() {
  char *rv;
  char *test_str;
  ...
  mu_assert("should return a cropped string",
            0 == strcmp(rv, "ENGQVIST,JOHANSSON,89"));
  free(test_str);

  /* Without checksum */
  test_str = strdup("$GPGGA,ENGQVIST,JOHANSSON,89\r\n");
  rv = _crop_sentence(test_str, strlen(test_str));
  mu_assert("should return a cropped string without checksum",
            0 == strcmp(rv, "ENGQVIST,JOHANSSON,89"));
  free(test_str);
  ...
  test_str = strdup("$GPGGA,,ENGQVIST,,JOHANSSON,,89,,\r\n");
  rv = _crop_sentence(test_str, strlen(test_str));
  mu_assert("should work with empty values",
            0 == strcmp(rv, ",ENGQVIST,,JOHANSSON,,89,,"));
  free(test_str);
  ...
  rv = _crop_sentence(test_str, strlen(test_str));
  mu_assert("should work with empty values and checksum",
            0 == strcmp(rv, ",ENGQVIST,,JOHANSSON,,89,,"));
  free(test_str);
