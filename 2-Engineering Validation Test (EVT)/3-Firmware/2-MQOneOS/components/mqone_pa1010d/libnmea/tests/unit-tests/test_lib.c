...
static char *test_get_type_ok() {
  ...
  res = nmea_get_type(sentence);
  mu_assert("should return correct type (GPGGA) even if talker is GN",
            NMEA_GPGGA == res);
  free(sentence);
...
static char *test_get_type_unknown() {
  char *sentence;
  nmea_t res;

  sentence = strdup("THISISWRONG");
  res = nmea_get_type(sentence);
  mu_assert("should return NMEA_UNKNOWN on unknown sentence type",
            NMEA_UNKNOWN == res);
  free(sentence);
  ...
  sentence = strdup("");
  res = nmea_get_type(sentence);
  mu_assert("should return nmea_unknown on empty sentence",
            NMEA_UNKNOWN == res);
  free(sentence);
...
static char *test_get_checksum_with_crc() {
...
static char *test_get_checksum_without_crc() {
...
static char *test_has_checksum_yes() {
...
static char *test_has_checksum_no() {
  // Sentence without checksum
  char *sentence = strdup("$GPGLL,4916.45,N,12311.12,W,225444,A\r\n");
  int res = nmea_has_checksum(sentence, strlen(sentence));
  mu_assert("should return -1 when sentence does not have a checksum",
            -1 == res);
  free(sentence);
...
static char *test_validate_ok_with_crc() {
...
static char *test_validate_ok_without_crc() {
  char *sentence;
  int res;
  ...
  mu_assert("should return 0 when check_checksum is 0 and crc is invalid",
            0 == res);
  free(sentence);
...
static char *test_validate_fail_type() {
...
static char *test_validate_fail_start() {
...
static char *test_validate_fail_end() {
  char *sentence;
  int res;
...
static char *test_validate_fail_empty() {
...
static char *test_parse_unknown() {
  char *sentence;
  nmea_s *res;
...
static char *test_parse_invalid() {
  char *sentence;
  nmea_s *res;
