...
static int txProcess(shtp_t *pShtp, uint8_t chan, const uint8_t *pData,
                     uint32_t len) {
  int status = SH2_OK;
...
static void rxAssemble(shtp_t *pShtp, uint8_t *in, uint16_t len,
                       uint32_t t_us) {
  uint16_t payloadLen;
  bool continuation;
  uint8_t chan = 0;
  ...
  memcpy(pShtp->inPayload + pShtp->inCursor, in + SHTP_HDR_LEN,
         len - SHTP_HDR_LEN);
  ...
  if (pShtp->inRemaining == 0) {
    ...
    if (pShtp->chan[chan].callback != 0) {
      pShtp->chan[chan].callback(pShtp->chan[chan].cookie, pShtp->inPayload,
                                 pShtp->inCursor, pShtp->inTimestamp);
    }
  }
}
...
int shtp_listenChan(void *pInstance, uint8_t channel, shtp_Callback_t *callback,
                    void *cookie) {
  shtp_t *pShtp = (shtp_t *)pInstance;
...
int shtp_send(void *pInstance, uint8_t channel, const uint8_t *payload,
              uint16_t len) {
  shtp_t *pShtp = (shtp_t *)pInstance;
...
void shtp_service(void *pInstance) {
  ...
  int len = pShtp->pHal->read(pShtp->pHal, pShtp->inTransfer,
                              sizeof(pShtp->inTransfer), &t_us);
  if (len > 0) {
