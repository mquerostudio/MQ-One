...
static char *test_position_parse_ok() {
  int res;
  char *s;
...
static char *test_position_parse_fail() {
  int res;
  char *s;
  nmea_position *pos = malloc(sizeof(nmea_position));
...
static char *test_cardinal_dir_parse_ok() {
  nmea_cardinal_t card;

  /* North */
  card = nmea_cardinal_direction_parse("N");
  mu_assert("should return NMEA_CARDINAL_NORTH on \"N\"",
            NMEA_CARDINAL_DIR_NORTH == card);

  /* East */
  card = nmea_cardinal_direction_parse("E");
  mu_assert("should return NMEA_CARDINAL_EAST on \"E\"",
            NMEA_CARDINAL_DIR_EAST == card);

  /* South */
  card = nmea_cardinal_direction_parse("S");
  mu_assert("should return NMEA_CARDINAL_SOUTH on \"S\"",
            NMEA_CARDINAL_DIR_SOUTH == card);

  /* West */
  card = nmea_cardinal_direction_parse("W");
  mu_assert("should return NMEA_CARDINAL_WEST on \"W\"",
            NMEA_CARDINAL_DIR_WEST == card);

  return 0;
}

static char *test_cardinal_dir_parse_fail() {
  nmea_cardinal_t card;

  /* Empty string */
  card = nmea_cardinal_direction_parse("");
  mu_assert("should return NMEA_CARDINAL_UNKNOWN on empty string",
            NMEA_CARDINAL_DIR_UNKNOWN == card);

  /* NULL */
  card = nmea_cardinal_direction_parse(NULL);
  mu_assert("should return NMEA_CARDINAL_UNKNOWN on NULL",
            NMEA_CARDINAL_DIR_UNKNOWN == card);

  /* Unknown character (J) */
  card = nmea_cardinal_direction_parse("J");
  mu_assert("should return NMEA_CARDINAL_UNKNOWN on \"J\"",
            NMEA_CARDINAL_DIR_UNKNOWN == card);

  /* Unknown character (n) */
  card = nmea_cardinal_direction_parse("n");
  mu_assert("should return NMEA_CARDINAL_UNKNOWN on \"n\"",
            NMEA_CARDINAL_DIR_UNKNOWN == card);

  return 0;
}

static char *test_time_parse_ok() {
  int rv;
  struct tm t;
...
static char *test_time_parse_fail() {
  int rv;
  struct tm t;
